...
import type { ChartOptions, EpisodeInfos, RatingsDisplayProps } from "@/types/All"
...
const getRatingTier = (rating: number | undefined, format: "1decimal" | "2decimal" = "2decimal") => {
	if (rating === undefined || rating === 0) return { label: "Unrated", color: "#6b7280", textColor: "#ffffff" }

	// Round based on display format to match what user sees
	const outOf10 = rating / 10
	const roundedValue = format === "1decimal"
	...
	if (roundedValue >= 9.3) return { label: "Awesome", color: "#166534", textColor: "#ffffff" }
	if (roundedValue >= 8.5) return { label: "Great", color: "#22c55e", textColor: "#4e4444" }
	if (roundedValue >= 7.5) return { label: "Good", color: "#eab308", textColor: "#4e4444" }
	if (roundedValue >= 6.5) return { label: "Regular", color: "#f97316", textColor: "#4e4444" }
	if (roundedValue >= 5.0) return { label: "Bad", color: "#ef4444", textColor: "#ffffff" }
...
export default function RatingsDisplay({
	...
	const nbOfTicks = results.length > 1000 ? 50 : results.length >= 500 ? 20 : results.length > 250 ? 10 : 5
	...
	const sortedResults = useMemo(() => {
		...
		sorted = sorted
			.filter(result => {
				if (options.filterFillerAndRecap === "remove" && (result.filler || result.recap)) {
					return false
				}
				return options.filterBelowScore.type === "remove" ? result.ratingAllStars !== undefined && result.ratingAllStars > Number(options.filterBelowScore.score) : true
			})
			.map(result => {
				let currentSaga = ""
				let currentArc = ""
				if (isOnePieceOnly) {
					const sagas = entryType === "manga" ? onePieceSagasChapters(results) : onePieceSagasEpisodes(results)
					for (const saga of sagas) {
						for (const arc of saga.story_arcs) {
							const ranges = arc.episodesOrChapters.split(",").map(range => range.trim())
				...
				const shouldHide = options.filterBelowScore !== undefined && options.filterBelowScore.type === "hide" && (
					(result.ratingAllStars !== undefined && result.ratingAllStars < Number(options.filterBelowScore.score)) ||
					(result.ratingFiveStars !== undefined && result.ratingFiveStars < Number(options.filterBelowScore.score))
				)

				return {
					...result,
					ratingFiveStars:
						(options.hideZeroValues && result.ratingFiveStars === 0) || (options.filterFillerAndRecap === "hide" && (result.filler || result.recap)) || shouldHide
							? undefined
							: result.ratingFiveStars,
					ratingAllStars:
						(options.hideZeroValues && result.ratingAllStars === 0) || (options.filterFillerAndRecap === "hide" && (result.filler || result.recap)) || shouldHide
							? undefined
		...
		return sorted
	}, [results, options.sortBy, options.hideZeroValues, options.filterFillerAndRecap, options.filterBelowScore, animeInfo?.mal_id])

	// Helper to round rating based on display format (returns value in /10 scale)
	const roundRating = (rating: number | undefined, format: "1decimal" | "2decimal"): number | undefined => {
	...
	const chartData = useMemo(() => {
		return sortedResults.map(item => ({
			...
			ratingAllStarsChart: roundRating(item.ratingAllStars, options.ratingDisplayFormat),
			// Keep five stars as percentage for right Y-axis
			ratingFiveStarsChart: item.ratingFiveStars === 0 ? undefined : item.ratingFiveStars,
		}))
	}, [sortedResults, options.ratingDisplayFormat])

	// Calculate moving average for trend line
	const trendLineData = useMemo(() => {
		if (!options.showTrendLine || options.sortBy !== "episodeNb") return chartData

		// Window size is proportional to data length (roughly 10-15% of total)
		const windowSize = Math.max(3, Math.min(Math.ceil(chartData.length * 0.1), 20))
	...
	useEffect(() => {
		if (options.filterBelowScore.type === "highlight" && options.filterBelowScore.score !== "") {
			toast.info(`Filtered ${sortedResults.filter(result => result.ratingAllStars !== undefined && result.ratingAllStars < Number(options.filterBelowScore.score)).length} out of ${results.length} ${entryType === "anime" ? "episodes" : "chapters"}`)
		} else if (options.filterBelowScore.type === "hide") {
			toast.info(`Hidden ${sortedResults.filter(result => result.ratingAllStars === undefined).length} out of ${results.length} ${entryType === "anime" ? "episodes" : "chapters"}`)
		} else if (options.filterBelowScore.type === "remove") {
			toast.info(`Removed ${results.length - sortedResults.length} out of ${results.length} ${entryType === "anime" ? "episodes" : "chapters"}`)
		}
	}, [sortedResults, entryType, options.filterBelowScore.type, results.length, options.filterBelowScore.score])
	...
	const CustomTooltip = ({ active, payload, label }: { active: boolean | undefined; payload: any; label: string | boolean }) => {
		if (active && payload && payload.length) {
			...
			const formatValue = (val: number | undefined) =>
				val !== undefined ? (options.ratingDisplayFormat === "1decimal" ? val.toFixed(1) : val.toFixed(2)) : "-"
			...
			return (
				<div className="border border-slate-200 p-2 rounded-md bg-white text-sm">
					...
					{fetchingMethod === "cheerioParser" && payload[0].payload.nbOfVotes && (
					...
					{fetchingMethod === "cheerioParser" && options.visibleRatingInfo.ratingFiveStars && fiveStarsValue && (
						<p className="label">{`5☆ rating: ${fiveStarsValue.toFixed(1)}%`}</p>
					)}
					{payload[0].payload.title && <p className="label">{`Title: ${payload[0].payload.title}`}</p>}
					...
					{payload[0].payload.currentSaga && <p className="label">{`Saga: ${payload[0].payload.currentSaga}`}</p>}
					{payload[0].payload.currentArc && <p className="label">{`Arc: ${payload[0].payload.currentArc}`}</p>}
				</div>
			)
		}
	...
	const handleTableClickSort = (sortBy: ChartOptions["sortBy"]) => {
		setOptions(prevOptions => ({ ...prevOptions, sortBy: prevOptions.sortBy === sortBy ? "episodeNb" : sortBy }))
	}
	...
	const hasZeroValues = (type: "ratingFiveStars" | "ratingAllStars" | "all") =>
		results.some(r => (type === "all" ? !r.ratingFiveStars || !r.ratingAllStars : type === "ratingFiveStars" ? !r.ratingFiveStars : !r.ratingAllStars))

	const calculateAverage = (key: "ratingFiveStars" | "ratingAllStars") => {
		const filteredResults = sortedResults.filter(result => (options.filterFillerAndRecap === "hide" ? result[key] !== undefined : true))
		return filteredResults.length > 0
			? (filteredResults.reduce((sum, result) => sum + (result[key] as number), 0) / filteredResults.length).toFixed(1)
			: "0"
	}

	const [avgRatingFiveStars, avgRatingAllStars] = ["ratingFiveStars", "ratingAllStars"].map(key =>
		calculateAverage(key as "ratingFiveStars" | "ratingAllStars"),
	)
		...
		<>
					...
					<div className="flex flex-col sm:flex-row items-start">
						{animeInfo.images?.webp?.image_url && (
							<Image
								...
								alt={animeInfo.titles.find(title => title.type === "Default")?.title || "Entry cover"}
								width={100}
								height={150}
						...
						<div className="flex-grow w-full">
							<div className="flex items-center gap-2 mb-2">
								<h2 className="text-xl font-bold">
									<a href={animeInfo.url} target="_blank" rel="noopener noreferrer" className="hover:underline">
							...
							<div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
								{[
									...
									{ label: entryType === "anime" ? "Episodes" : "Chapters", value: entryType === "anime" ? animeInfo.episodes : animeInfo.chapters || "-" },
									{ label: "Status", value: animeInfo.status },
									{ label: "Aired", value: entryType === "anime" ? animeInfo.aired?.string : animeInfo.published?.string || "-" },
									{ label: "Score", value: `${animeInfo.score} (${animeInfo.scored_by?.toLocaleString("en-US") || "-"} users)` },
			...
			{options.viewMode === "wrapped" ? (
					...
					<TooltipProvider delayDuration={100}>
						<div className="flex flex-wrap gap-2">
							{sortedResults.map(result => {
								...
								const displayRating = formatRating(rating, options.ratingDisplayFormat)
								...
								return (
									<Tooltip key={result.episodeNb}>
										<TooltipTrigger asChild>
											<div
												className={cn(
													...
													isFillerOrRecap && options.filterFillerAndRecap === "highlight" && "ring-2 ring-red-500"
												)}
												style={{ backgroundColor: tier.color }}
											>
												<span className="text-[14px] font-medium opacity-80 w-full text-left" style={{ color: tier.textColor }}>
													{entryType === "anime" ? "E" : "Ch"}{result.episodeNb}
												</span>
												<span className="text-2xl font-bold w-full text-right" style={{ color: tier.textColor }}>
													{displayRating}
												</span>
											</div>
										</TooltipTrigger>
										<TooltipContent className="max-w-[280px]">
											<div className="block">
												<span>
													{entryType === "anime" ? "Episode" : "Chapter"} {result.episodeNb}
												</span>
												{result.title && (
											...
											{result.aired && (
												<span className="text-xs text-muted-foreground">
													Aired: {new Date(result.aired).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}
												</span>
											)}
										</TooltipContent>
									</Tooltip>
				...
				<ChartContainer config={chartConfig}>
					<LineChart accessibilityLayer data={trendLineData} margin={{ top: 20, left: 12, right: 12, bottom: 20 }}>
						...
						<XAxis
							...
							ticks={
								isLongAnime && !episodeCount
									? [
										1,
										...Array.from({ length: Math.ceil(results[results.length - 1].episodeNb / nbOfTicks) }, (_, i) =>
											Math.min((i + 1) * nbOfTicks, results[results.length - 1].episodeNb),
										),
									]
									: undefined
							}
							label={{ value: entryType === "anime" ? "Episode" : "Chapter", position: "bottom", offset: 10 }}
						/>
						<YAxis
							yAxisId="left"
							tickLine={false}
							axisLine={false}
							tickMargin={8}
							tickFormatter={(value) => options.yAxisDomain === "full" ? value.toFixed(0) : options.ratingDisplayFormat === "1decimal" ? value.toFixed(1) : value.toFixed(2)}
							label={{ value: "Rating (/10)", angle: -90, position: "insideLeft", offset: 0 }}
							domain={options.yAxisDomain === "full" ? [0, 10] : ["dataMin-0.05", "dataMax+0.05"]}
						/>
						{options.visibleRatingInfo.ratingFiveStars && (
							<YAxis
								...
								label={{ value: "5☆ Rating (%)", angle: 90, position: "insideRight", offset: 0 }}
								domain={options.yAxisDomain === "full" ? [0, 100] : ["dataMin-0.05", "dataMax+0.05"]}
							/>
						)}
						<ChartTooltip content={<CustomTooltip active payload label />} />
						{!hasZeroValues("ratingFiveStars") && options.visibleRatingInfo.ratingFiveStars && (
							<ReferenceLine yAxisId="right" y={Number(avgRatingFiveStars)} stroke="var(--color-ratingFiveStars)" strokeDasharray="3 3" />
						)}
						{!hasZeroValues("ratingAllStars") && options.visibleRatingInfo.ratingAllStars && (
							<ReferenceLine yAxisId="left" y={Number(avgRatingAllStars) / 10} stroke="var(--color-ratingAllStars)" strokeDasharray="3 3" />
						)}
						{options.filterBelowScore.type === "highlight" && options.filterBelowScore.score && (
							<ReferenceLine yAxisId="left" y={Number(options.filterBelowScore.score) / 10} stroke="#fb923c" strokeDasharray="3 3" label={`Below ${formatRating(Number(options.filterBelowScore.score), options.ratingDisplayFormat)}/10`} />
						)}
						...
						{options.visibleRatingInfo.ratingAllStars && (
							<Line
								...
								strokeWidth={2}
								dot={props => {
									const isFillerOrRecap = props.payload.filler || props.payload.recap
									const filterThreshold = Number(options.filterBelowScore.score) / 10
									const isBelowScoreAndHighlighted = props.payload.ratingAllStarsChart !== undefined &&
										props.payload.ratingAllStarsChart < filterThreshold &&
										options.filterBelowScore.type === "highlight"
									const isHighlightedFiller = isFillerOrRecap && options.filterFillerAndRecap === "highlight"
									...
									return (
										<circle
											...
											cy={props.cy}
											r={4}
											fill={isHighlightedFiller ? "#ef4444" : isBelowScoreAndHighlighted ? "#fb923c" : "hsl(var(--chart-3))"}
										/>
									)
								}}
								// biome-ignore lint/suspicious/noExplicitAny: <explanation>
								activeDot={(props: any) => {
									...
									return (
										<circle
											...
											cy={props.cy}
											r={6}
											fill={isFillerOrRecap && options.filterFillerAndRecap === "highlight" ? "#ef4444" : isBelowScoreAndHighlighted ? "#fb923c" : "hsl(var(--chart-3))"}
										/>
									)
								}}
							>
								<LabelList
									...
									fontSize={12}
									formatter={(value: number) => options.ratingDisplayFormat === "1decimal" ? value.toFixed(1) : value.toFixed(2)}
								/>
							</Line>
						)}
						{options.visibleRatingInfo.ratingFiveStars && (
				...
				<>
					<Table>
						<TableHeader>
							<TableRow>
								...
								>
									...
									{options.sortBy === "episodeNb" && <ChevronDown className="inline-block ml-1" size={16} />}
								</TableHead>
								{options.visibleRatingInfo.ratingAllStars && (
									<TableHead
									...
									>
										Rating (/10)
										{options.sortBy === "ratingAllStars" && <ChevronDown className="inline-block ml-1" size={16} />}
									</TableHead>
								)}
								{options.visibleRatingInfo.ratingFiveStars && (
									<TableHead
									...
									>
										5☆ rating (%)
										{options.sortBy === "ratingFiveStars" && <ChevronDown className="inline-block ml-1" size={16} />}
									</TableHead>
								)}
								{/* {options.visibleRatingInfo.ratingAllStarsRounded && (
								...
								{fetchingMethod === "cheerioParser" && <TableHead># of votes</TableHead>}
								...
								{(fetchingMethod === "cheerioParser" || (isOnePieceOnly && entryType === "manga")) && (
						...
						<TableBody>
							{sortedResults.map(result => {
								const isBelowScoreAndHighlighted = result.ratingAllStars !== undefined && result.ratingAllStars < Number(options.filterBelowScore.score) && options.filterBelowScore.type === "highlight"
								const isFillerOrRecap = result.filler || result.recap
								const ratingCellClasses = cn(
									isFillerOrRecap && options.filterFillerAndRecap === "highlight" && "text-red-500",
									...
									<TableRow key={result.episodeNb}>
										...
										{options.visibleRatingInfo.ratingAllStars && (
											<TableCell className={ratingCellClasses}>
												{formatRating(result.ratingAllStars, options.ratingDisplayFormat)}
											</TableCell>
										)}
										{options.visibleRatingInfo.ratingFiveStars && (
											<TableCell className={ratingCellClasses}>
												{result.ratingFiveStars !== undefined && result.ratingFiveStars !== 0 ? `${result.ratingFiveStars.toFixed(1)}%` : "-"}
											</TableCell>
										)}
										...
										{fetchingMethod === "cheerioParser" && (
											<TableCell>{result.nbOfVotes !== undefined ? result.nbOfVotes.toLocaleString("en-US") : "-"}</TableCell>
										)}
										<TableCell className="max-w-[200px]">
											{result.title || "-"}
											{result.filler && <span className="ml-2 text-xs font-semibold text-red-500">(Filler)</span>}
											{result.recap && <span className="ml-2 text-xs font-semibold text-blue-500">(Recap)</span>}
										</TableCell>
										<TableCell>
											{result.aired
												? new Date(result.aired).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" })
												: "-"}
										</TableCell>
										...
										<TableCell>
											{result.forumTopicUrl !== undefined ? (
												<a href={result.forumTopicUrl} target="_blank" rel="noopener noreferrer" className="hover:text-blue-500">
			...
			<div className="max-2xl:mt-7">
				<dl className="mt-1 grid grid-cols-1 gap-4 sm:grid-cols-3 max-w-[600px]">
					{[
						{ type: options.visibleRatingInfo.ratingAllStars, label: "Average ratings", value: avgRatingAllStars, key: "ratingAllStars" },
							...
							item.type && (
								<div key={item.key} className="rounded-lg bg-white p-3 shadow">
									...
									{hasZeroValues(item.key as "ratingFiveStars" | "ratingAllStars" | "all") ? (
										<p className="text-sm italic mt-1">Can&apos;t calculate exact average, because some episodes are not rated.
											{item.value !== undefined && (item.isPercentage
										...
										<dd className="mt-1 text-md font-semibold tracking-tight text-gray-900">
											{item.isPercentage ? `${Number(item.value).toFixed(1)}%` : `${formatRating(Number(item.value), options.ratingDisplayFormat)}/10`}
										</dd>
									)}
								</div>
							),
				...
				{isLongAnime && (
					<RatingsDisplayAdditionalGraph results={results} animeId={animeInfo?.mal_id || 0} isOnePieceOnly={isOnePieceOnly} entryType={entryType} options={options} />
				)}
			</div>
		</>
	)
