...
export function SagaAndArcsRatings({ results, entryType }: { results: EpisodeInfos[]; entryType: EntryType }) {
	const [mode, setMode] = useState<"saga" | "arc">("arc")
	const [filterFiller, setFilterFiller] = useState<boolean>(true)
	const [hideFillerFromAverage, setHideFillerFromAverage] = useState<boolean>(true)
	const [sortBy, setSortBy] = useState<"rating" | "chronological">("chronological")

	const calculateAverageScore = (episodes: EpisodeInfos[]) => {
		const filteredEpisodes = hideFillerFromAverage ? episodes.filter(episode => !episode.filler) : episodes
		return filteredEpisodes.reduce((sum, episode) => sum + episode.score, 0) / filteredEpisodes.length
	}
	...
	const sagaData = onePieceSagas.map(saga => {
		const sagaEpisodes = results.filter(
			episode => (saga.minAiredDate ? episode.aired >= saga.minAiredDate : true) && (saga.maxAiredDate ? episode.aired <= saga.maxAiredDate : true),
		)
		...
		saga.story_arcs.map(arc => {
			...
			const arcEpisodesOrChapters = results.filter(episode => {
				...
				return episodeRanges.some(range => {
					...
					return entryType === "anime" ? episode.episodeNb === Number(range) : episode.episodeNb >= Number(range) && episode.episodeNb <= Number(range)
				})
			})
			...
			return {
				...
				filler: arcEpisodesOrChapters.length > 0 && arcEpisodesOrChapters.every(episode => episode.filler),
			}
		}),
	)

	const filteredData = mode === "saga" ? sagaData : arcData.filter(item => !filterFiller || !item.filler)
		...
		if (active && payload && payload.length) {
			...
			return (
				<div className="border border-slate-200 p-2 rounded-md bg-white">
					<p className="label">{`${mode === "saga" ? "Saga" : "Arc"}: ${label} ${data.filler ? "(Filler)" : ""}`}</p>
					{mode === "saga" && <p className="label">{`Arcs: ${data.arcs}`}</p>}
					{mode === "arc" && <p className="label">{`Saga: ${data.saga}`}</p>}
					<p className="label">{`Average rating: ${data.averageScore}/5`}</p>
					<p className="label">{`Number of ${entryType === "anime" ? "episodes" : "chapters"}: ${data.episodeCount}`}</p>
					<p className="label">{`${entryType === "anime" ? "Episodes" : "Chapters"}: ${data.episodeRange}`}</p>
					<p className="label">{`Aired dates: ${data.airDateRange}`}</p>
				</div>
			)
		}
		return null
	...
	return (
		<>
			<h3 className="text-xl font-semibold mt-5 mb-3">Average score by {mode === "saga" ? "saga" : "arc"}</h3>
			<div className="flex gap-4 mb-4 flex-wrap">
				<div className="flex flex-col">
					<label htmlFor="mode-select">Mode</label>
					<Select value={mode} onValueChange={(value: "saga" | "arc") => setMode(value)}>
				...
				<div className="flex flex-col">
					...
					<Select value={sortBy} onValueChange={(value: "rating" | "chronological") => setSortBy(value)}>
						<SelectTrigger className="w-[140px]  focus:ring-offset-1 focus:ring-2 bg-white">
							<SelectValue placeholder="Sort by" />
						</SelectTrigger>
						<SelectContent>
							<SelectItem value="chronological">{mode === "saga" ? "Saga" : "Arc"}</SelectItem>
				...
				{entryType === "anime" && (
					<div className="flex flex-col">
						<label htmlFor="hide-filler-from-average">Count fillers in avg score?</label>
						<Select value={hideFillerFromAverage ? "true" : "false"} onValueChange={value => setHideFillerFromAverage(value === "true")}>
							<SelectTrigger className="w-[140px] focus:ring-offset-1 focus:ring-2 bg-white">
			...
			<ChartContainer config={chartConfig}>
				<LineChart data={sortedData} margin={{ top: 20, left: 20, right: 20, bottom: 60 }}>
					...
					<YAxis
						...
						domain={[4, 4.8]}
						// ticks={[4, 4.5, 4.8]}
						label={{ value: "Average Score (/5)", angle: -90, position: "insideLeft", offset: 0 }}
					/>
					...
					<Line
						...
						dot={(props) => {
							...
							return <circle cx={props.cx} cy={props.cy} r={4} fill={isFiller ? "hsl(var(--destructive))" : "hsl(var(--chart-3))"} />;
						}}
						// biome-ignore lint/suspicious/noExplicitAny: <explanation>
						activeDot={(props: any) => {
							const isFiller = props.payload.filler;
							return <circle cx={props.cx} cy={props.cy} r={6} fill={isFiller ? "hsl(var(--destructive))" : "hsl(var(--chart-3))"} />;
						}}
					>
