		...
		const fetchTopAnime = async () => {
			...
			try {
				const response = await fetch(
					`https://api.jikan.moe/v4/top/anime?type=tv&filter=${filter}&limit=9`,
				);
				if (!response.ok) {
						...
						.map((anime) => (
							...
							>
								<div className="relative aspect-[3/4] w-full overflow-hidden">
									...
									/>
									{anime.score && (
										<div className="absolute right-2 top-2 rounded bg-white px-2 py-1 text-sm font-bold text-gray-600">
