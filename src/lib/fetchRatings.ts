...
// const corsProxyUrl = "https://api.cors.lol/?";
...
export async function fetchRatings(
	...
	const forumUrlProxy = `${corsProxyUrl}${encodeURIComponent(forumUrl)}`;
	...
	const limitedPageCount = episodeCount
		? Math.min(pageCount, Math.ceil(episodeCount / 50))
		: pageCount;

	for (let page = 1; page <= limitedPageCount; page++) {
		const pageUrl = `${forumUrl}&show=${(page - 1) * 50}`;
		const pageUrlProxy = `${corsProxyUrl}${encodeURIComponent(pageUrl)}`;
	...
	const limitedPollLinks = episodeCount
		? filteredPollLinks.slice(0, episodeCount)
		: filteredPollLinks;
	...
	for (const { forumTopicUrl: pollUrl, episodeNb } of limitedPollLinks) {
		...
		const pollUrlProxy = `${corsProxyUrl}${encodeURIComponent(pollUrl)}`;
