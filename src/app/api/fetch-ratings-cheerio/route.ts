...
export async function GET(request: Request) {
	...
	try {
		...
		const startAfter = searchParams.get("startAfter")
			? Number.parseInt(searchParams.get("startAfter")!)
			: null;
		...
		if (!forumResponse.data) {
			...
			return NextResponse.json(
				{ error: "No data received from the forum request" },
				{ status: 500 },
			);
		}
		...
		const limitedPageCount = episodeCount
			? Math.min(pageCount, Math.ceil(Number.parseInt(episodeCount) / 50))
			: pageCount;
		...
		for (let page = 1; page <= limitedPageCount; page++) {
			...
			$forumNextPage('tr[id^="topicRow"]').each((_, row) => {
				const episodeDiscussionLink = $forum(row)
					.find("td:nth-child(2) a")
					.attr("href");
				const episodeDiscussionText = $forum(row)
					.find("td:nth-child(2) a")
					.first()
					.text();
				const episodeMatch = episodeDiscussionText.match(
					/(?:Episode|Chapter)\s+(\d+)(?:\s+Discussion)?/i,
				);
				if (episodeMatch?.[1]) {
					const episodeNb = Number.parseInt(episodeMatch[1], 10);
					// console.log("episode", episode)
					pollLinks.push({
						forumTopicUrl: `https://myanimelist.net${episodeDiscussionLink}&pollresults=1`,
						episodeNb,
					});
				}
			});
		...
		const limitedPollLinks = episodeCount
			? filteredPollLinks.slice(0, Number.parseInt(episodeCount))
			: filteredPollLinks;

		const averageFetchTime =
			fetchTimes.reduce((a, b) => a + b, 0) / fetchTimes.length;
		const estimatedTotalTime =
			(averageFetchTime * limitedPollLinks.length) / 1000; // in seconds
		...
		writer.write(
			encoder.encode(
				JSON.stringify({
					estimatedTotalTime: Number.parseFloat(estimatedTotalTime.toFixed(2)),
				}),
			),
		);
		...
		const processEpisodes = async () => {
			...
			for (let i = 0; i < limitedPollLinks.length; i++) {
				...
				// Date of first post: .messages > first .forum-topic-message > .message-header > .date [data-time]
				const dateOfFirstPost = $poll(".messages")
					.find(".forum-topic-message")
					.first()
